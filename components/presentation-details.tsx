...
const formSchema = z.object({
  ...
  ppt: z.custom<File>((val) => val instanceof File, "PPT is required"),
})
...
export const PresentationDetails = ({
  ...
  const onSubmit = async () => {
    ...
    formData.append("conclusion", itemData.conclusion);
    formData.append("inspiration", itemData.inspiration);
    const response = await axios.patch(`/api/item/${item.id}`, formData)
    if (!response || response.status !== 200) {
      toast.error("Failed to update item!")
      return
    } else {
      const itemInfo: Item = response.data;
      if (!itemInfo) {
        toast.error("Failed to update item");
        return
      }
        ...
        <div className="flex items-center justify-between">
          <div className="flex gap-x-8">
            <div className="flex items-center gap-x-1 text-sm text-muted-foreground">
              <CircleUserRound className="h-4 w-4" /> {item.username}
            </div>
            <div className="flex items-center gap-x-1 text-sm text-muted-foreground">
        ...
        {editable ? (
          <Form {...form}>
            ...
            <FormField
              ...
              <FormField
                ...
                render={() => (
                  <>
                    <FormLabel className="text-xl font-medium text-sky-500">PowerPoint</FormLabel>
                    <Dropzone
                      noClick
                      multiple={false}
                      onDropAccepted={(acceptedFiles) => {
                        if (acceptedFiles && acceptedFiles.length > 0) {
                          form.setValue("ppt", acceptedFiles[0])
          ...
          <>
            ...
            <div className="flex flex-col gap-y-2">
              ...
              >
                <BsFiletypePpt className="h-4 w-4 text-red-500 mr-1" />
